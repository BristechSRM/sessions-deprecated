...
type ProfilesController() = 
    ...
    member x.Patch(id: Guid, operations : RawPatchOperation list option) = 
        Log.Information("Received Patch request for profile with id: {id}")
        ...
        | Success ops -> 
            ...
            | Success () -> 
                ...
                x.Request.CreateResponse(HttpStatusCode.OK)
            | Failure error -> 
                Log.Error("Patch request profile with id {id} failed with status code: {code} and message: {message",id,error.HttpStatus,error.Message)
                x.Request.CreateErrorResponse(error.HttpStatus,error.Message)
        | Failure error -> 
